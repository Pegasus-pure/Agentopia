...
class RoleAgent:
    ...
    def _generate_with_functions(
        ...
        *,
        ...
        final_answer = last.get("content", "").strip() if last else ""
        think = ""  # think tags already stripped by generate_with_fc
    ...
    def signup_public_events(self, events: List["PublicEvent"]) -> List[str]:
        ...
        signup_data = []  # List of (event_name, phase)
        for blk in blocks:
            s = blk.strip()
            try:
                data = _json.loads(s)
                if isinstance(data, dict) and data.get("type") == "signup":
                    ...
                    if name:
                        signup_data.append((str(name), str(phase)))
    ...
    def _parse_exit_activity_response(
        ...
        if activity_type == "joint":
            ...
            # Find all "Reflection:" occurrences and use the last one (joint)
            reflection_matches = list(
                re.finditer(
                    r"[#*_]*\s*Reflection\s*[#*_]*\s*:\s*[#*_]*\s*(.+)",
            ...
            if reflection_matches:
                ...
                # Extract summary: from "Activity" to the last "Reflection:"
                summary_match = re.search(
                    r"[#*_]*\s*Activity\s*[#*_]*\s*:\s*\n(.+?)(?=\n\s*[#*_]*\s*Reflection\s*[#*_]*\s*:)",
                    response[
            ...
            # Find all "Reflection:" occurrences and use the last one (solo/public)
            reflection_matches = list(
                re.finditer(
                    r"[#*_]*\s*Reflection\s*[#*_]*\s*:\s*[#*_]*\s*(.+)",
                    response, re.DOTALL | re.IGNORECASE
    ...
    def enter_solo_activity(self, location_desc: Optional[str] = None) -> None:
        ...
        Args:
            location_desc: Optional description of current location (from schedule.location).
                           When provided, NPC can see where they are in the prompt.
        """
        t = self.clock.get_time()
        self.logger.info(
